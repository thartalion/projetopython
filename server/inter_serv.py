...
class Principal:
    def __init__(self, root):
        ...
        self.btnSend = ttk.Button(root, text='Enviar',
                                  command=lambda: self.resposta(arquivo.get(), endereco, portassl, portadados))
        self.btnSend.grid(row=5, column=0)
        self.btnQuit = ttk.Button(root, text='Sair', command=quit)
        self.btnQuit.grid(row=5, column=1)

        status = StringVar()
        self.txtStatus = ttk.Text(root, width=30, height = 100)
